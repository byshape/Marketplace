...
describe("Marketplace", function () {
  ...
  it("Should support IMarketplace1155 interface", async function () {
    let abi = [
      "function createItem1155(uint256,uint256)", "function listItem1155(uint256,uint256,uint256)",
      ...
      "function finishAuction1155(uint256)"
    ];
    let functions = ["createItem1155", "listItem1155", "cancelItem1155", "buylItem1155",
      "listItemOnAuction1155", "makeBid1155", "finishAuction1155"];
    expect(await marketplace.connect(admin).supportsInterface(await getInterface(abi, functions))).to.be.equal(true);
  });

  it("Should support IMarketplace721 interface", async function () {
    let abi = [
      "function createItem721(uint256)", "function listItem721(uint256,uint256)",
    ...
    let functions = ["createItem721", "listItem721", "cancelItem721", "buylItem721",
  ...
  it("Lists 1155 item", async () => {
    ...
    await expect(marketplace.connect(user).listItem1155(
      tokenId, amount, price.toString()
    )).to.emit(marketplace, "Item1155Listed").withArgs(tokenId, price, amount);
  ...
  it("Lists 1155 item on auction", async () => {
    ...
    )).to.emit(marketplace, "Item1155ListedAuction").withArgs(tokenId, price, amount);
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(amount);
    expect(await token1155.balanceOf(user.address, tokenId)).to.be.equal(0);

    await expect(marketplace.connect(user).listItemOnAuction1155(
      tokenId2, price.toString(), amount
    )).to.emit(marketplace, "Item1155ListedAuction").withArgs(tokenId2, price, amount);
    expect(await token1155.balanceOf(marketplace.address, tokenId2)).to.be.equal(amount);
    expect(await token1155.balanceOf(user.address, tokenId2)).to.be.equal(0);
  });
  ...
  it("Doesn not sell non-existent 1155 item", async () => {
    let amount: number = 1;
    let tokenId = 10;
    await expect(marketplace.connect(user2).buylItem1155(tokenId, amount)).to.be.revertedWith("DoesNotExist");
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(0);
    expect(await token1155.balanceOf(user2.address, tokenId)).to.be.equal(0);
  });

  it("Doesn not sell 1155 item if unsufficient balance", async () => {
    let amount: number = 1;
    let tokenId = 0;
    await expect(marketplace.connect(user3).buylItem1155(tokenId, amount)).to.be.revertedWith("InsufficientBalance");
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(4);
    expect(await token1155.balanceOf(user.address, tokenId)).to.be.equal(6);
  ...
  it("Sels 1155 item", async () => {
    let amount: number = 1;
    let tokenId = 0;
    await expect(marketplace.connect(user2).buylItem1155(tokenId, amount)).to.emit(marketplace, "Item1155Sold").withArgs(tokenId, amount, user2.address, price);;
  ...
  it("Accepts bid for 1155 item", async () => {
    ...
    await expect(marketplace.connect(user).makeBid1155(
      tokenId, bid.toString()
    )).to.emit(marketplace, "Item1155BiddedAuction").withArgs(tokenId, user.address, bid);
    await expect(marketplace.connect(user).makeBid1155(
      tokenId2, bid.toString()
    )).to.emit(marketplace, "Item1155BiddedAuction").withArgs(tokenId2, user.address, bid);
    expect(await token20.balanceOf(user.address)).to.be.equal(userBalance.sub(bid * BigInt(2)));
    
    await expect(marketplace.connect(user2).makeBid1155(
      tokenId, bid2.toString()
    )).to.emit(marketplace, "Item1155BiddedAuction").withArgs(tokenId, user2.address, bid2);
    await expect(marketplace.connect(user2).makeBid1155(
      tokenId2, bid2.toString()
    )).to.emit(marketplace, "Item1155BiddedAuction").withArgs(tokenId2, user2.address, bid2);
    expect(await token20.balanceOf(user.address)).to.be.equal(userBalance);
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(marketplaceBalance);

    await expect(marketplace.connect(user2).makeBid1155(
      tokenId2, bid3.toString()
    )).to.emit(marketplace, "Item1155BiddedAuction").withArgs(tokenId2, user2.address, bid3);
    expect(await token20.balanceOf(user2.address)).to.be.equal(userBalance2.sub(bid2 + bid3));
  ...
  it("Does not finish auction for 1155 item before it ends", async () => {
    let tokenId: number = 1;
    let marketplaceBalance = await token1155.balanceOf(marketplace.address, tokenId);
    await expect(marketplace.connect(user2).finishAuction1155(tokenId)).to.be.revertedWith("WrongPeriod");
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(marketplaceBalance);
  });
  ...
  it("Does not finish auction for non-existent 1155 item", async () => {
    let tokenId: number = 100;
    let marketplaceBalance = await token1155.balanceOf(marketplace.address, tokenId);
    await expect(marketplace.connect(user2).finishAuction1155(tokenId)).to.be.revertedWith("DoesNotExist");
    expect(await token1155.balanceOf(marketplace.address, tokenId)).to.be.equal(marketplaceBalance);
  });
  ...
  it("Finishes auction for 1155 item, 2 or less bids", async () => {
    ...
    await ethers.provider.send('evm_increaseTime', [auctionDuration]);
    let marketplaceBalance = await token1155.balanceOf(marketplace.address, tokenId);
    await expect(marketplace.connect(user2).finishAuction1155(tokenId)).to.emit(marketplace, "Item1155FinishedAuction").withArgs(tokenId, user.address, price * BigInt(5 * 3));
    expect(await token1155.balanceOf(user.address, tokenId)).to.be.equal(marketplaceBalance);
  });

  it("Finishes auction for 721 item, more than 2 bids", async () => {
    let tokenId: number = 2;
    let marketplaceBalance = await token1155.balanceOf(marketplace.address, tokenId);
    await expect(marketplace.connect(user2).finishAuction1155(tokenId)).to.emit(marketplace, "Item1155FinishedAuction").withArgs(tokenId, user2.address, price * BigInt(5 * 4));
